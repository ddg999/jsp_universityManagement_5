	...
	<div class="sub--menu">
		...
		<div class="sub--menu--mid">
			<table class="sub--menu--table" border="1">
				<tr>
					<td><a
						href="${pageContext.request.contextPath}/sugang/subject">강의
							시간표 조회</a></td>
				</tr>
				<tr>
					<td><a
	...
	<main>
		...
		<div class="d-flex justify-content-between align-items-start">
			...
			<div class="sub--filter">
				<form action="/sugang/preRegist/search" method="get">
					<div>
						<!-- 강의구분 콤보박스 -->
							...
							<tr>
								...
								<td class="sub--list--button--row"><c:choose>
										...
										<c:when test="${subject.status == true}">
											<form
												action="/sugang/pre/delete?subjectId=${subject.id}&call=list"
												method="post">
										...
										<c:when test="${subject.numOfStudent == subject.capacity}">
											<button type="button"
												style="background-color: #c5b; color: white" disabled>마감</button>
										</c:when>
										<%-- 신청되지 않은 상태라면 --%>
										<c:otherwise>
											<form
												action="/sugang/pre/add?subjectId=${subject.id}&call=list"
												method="post">
												<input type="hidden" name="studentId"
													value="${principal.id}">
												<button type="submit"
							...
							<c:otherwise>
								<c:choose>
									<c:when
										...
										<a
											href="${pageContext.request.contextPath}/sugang/preRegist/search?page=${i}&type=${selectedType}&deptName=${selectedDeptName}&name=${selectedName}">${i}</a>
									</c:when>
									<c:otherwise>
										<a
